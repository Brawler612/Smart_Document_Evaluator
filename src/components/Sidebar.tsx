...
const STUDENT_NAV = [
  { to: '/assignments', icon: ClipboardList, label: 'Submit work' },
      ...
      <nav className="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
        {nav.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} onClick={onClose}
            className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group ${isActive ? 'bg-[#ffd21a] text-[#84001B] shadow-md' : 'text-white/70 hover:bg-[#ffd21a]/25 hover:text-white'}`}>
            {({ isActive }) => (
              <>
                <Icon className={`w-[18px] h-[18px] flex-shrink-0 transition-colors ${isActive ? 'text-[#84001B]' : 'text-white/50 group-hover:text-[#ffd21a]'}`} />
