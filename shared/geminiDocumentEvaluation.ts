...
export function formatGeminiEvaluationError(err: unknown): string {
  if (err instanceof Error) {
    ...
    if (/No Gemini API key on the server|GEMINI_API_KEY is not set on the server/i.test(m)) {
      return 'Vercel has no Gemini API key for this deployment. Add GEMINI_API_KEY (or VITE_GEMINI_API_KEY) in the Vercel project linked to this URL, Production scope, redeploy.';
    }
